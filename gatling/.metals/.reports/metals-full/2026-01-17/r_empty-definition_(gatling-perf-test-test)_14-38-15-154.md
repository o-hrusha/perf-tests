...
non-local guesses:
	 -io/gatling/core/Predef.cartContentMap.
	 -io/gatling/core/Predef.cartContentMap#
	 -io/gatling/core/Predef.cartContentMap().
	 -io/gatling/http/Predef.cartContentMap.
	 -io/gatling/http/Predef.cartContentMap#
	 -io/gatling/http/Predef.cartContentMap().
	 -cartContentMap.
	 -cartContentMap#
	 -cartContentMap().
	 -scala/Predef.cartContentMap.
	 -scala/Predef.cartContentMap#
	 -scala/Predef.cartContentMap().
offset: 2675
...
object Cart {
  ...
  def placeAnOrder(): ChainBuilder = {
    exec { session =>
      ...
      val pIdParams = cartContentMap.map { case (id, _) => "p_id[]" -> id }.toSeq
      val pQuantityParams = cartContentMap.map { case (_, qty) => "p_quantity[]" -> qty.toString }.toSeq
    ...
    exec { session =>
  // ... (всё то же самое до formParams)

  println(s"cartContentMap: $cartContentMap@@")
  println(s"formParams: $formParams")
}
  }
